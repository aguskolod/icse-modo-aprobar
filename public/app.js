(() => {
  ...
  function renderHome() {
    ...
    const tasks = [
      ["lesson-menem", "Dominar Menem 1989-1999", "Leer, cerrar y explicar", "55 min", state.completedLessons.includes("menem")],
      ["lesson-alianza", "Reconstruir la crisis de 2001", "Memorizar la cadena causal", "60 min", state.completedLessons.includes("alianza")],
      ["quick", "Hacer 10 preguntas guiadas", "Leer cada explicación", "15 min", state.quickAttempts.length > 0],
      ["exam-p2-t1", "Rendir un Tema 1 real", "Sin mirar la clave", "35-60 min", Boolean(state.examResults["p2-t1"])]
    ];
    return `${pageHead("TABLERO", "Tu misión: asegurar el 4", "Tenés poco tiempo, así que el recorrido prioriza reconocimiento de tesis, procesos repetidos y práctica con el formato exacto del examen.", '<button class="button red" data-view="ruta">Ver qué hago hoy</button>')}
      ...
      <div class="today-list">${tasks.map(([id, name, detail, time, done], index) => `<div class="today-row ${done ? "done" : ""}"><button class="check" data-home-task="${id}" aria-label="Abrir tarea">${done ? "✓" : index + 1}</button><div><div class="task-name">${name}</div><div class="task-detail">${detail}</div></div><span class="task-time">${time}</span><button class="button small secondary" data-home-task="${id}">${done ? "Repasar" : "Abrir"}</button></div>`).join("")}</div>
  ...
  function renderRoute() {
    const days = [
      ["Día 1", "Reconstruir y atacar lo más probable", [["d1a", "90'", "Menem + línea de tiempo", "Lección crítica", "menem"], ["d1b", "75'", "Estado, régimen, democracia y nación", "Cuadro comparativo", "bases"], ["d1c", "45'", "Práctica guiada", "10 preguntas con explicación", "quick"], ["d1d", "60'", "Segundo parcial Tema 1", "Simulación real", "exam-p2-t1"]]],
      ["Día 2", "Segundo parcial a fondo", [["d2a", "60'", "Crisis de 2001", "Cadena causal completa", "alianza"], ["d2b", "70'", "Autores cortos", "Autor → tesis → trampa", "autores"], ["d2c", "90'", "Dos parciales del segundo", "Temas 2 y 3", "exam-p2-t2"], ["d2d", "30'", "Repaso de errores", "Sólo lo que fallaste", "mistakes"]]],
  ...
  function startExam(id) {
    ...
    if (!exam) return;
    quizSession = { type: "exam", title: exam.title, examId: exam.id, questions: exam.questions, index: 0, answers: {}, startedAt: Date.now(), duration: 90 * 60 };
